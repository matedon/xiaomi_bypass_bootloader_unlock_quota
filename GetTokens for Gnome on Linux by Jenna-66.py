...
firefox_cmd = r'gnome-terminal -- sh -c "firefox -P default-release %s"'
...
def prompt_login_firefox():
    ...
    try:
        subprocess.run(["killall", "-1", "firefox"], check=False)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    for i in range(1, 5):
        subprocess.Popen(
            f'gnome-terminal -- sh -c "echo {i} | python {script_path}"',
            shell=True
        )
        time.sleep(0.1)
    time.sleep(0.5)
